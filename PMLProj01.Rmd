...
The result showed that the 4 preditors of this model are roll_belt, pitch_forearm, magnet_dumbbell_y and roll_forearm variables. It has taken approximately 16 seconds to build this model.
```{r}
time1<-proc.time()
...
This classification tree model has the poor accuracy(0.4488).
```{r}
confusionMatrix(wleTestSS$classe, predict(ctreeFit, wleTestSS))
```
##Building Model With Random Forest
The result showed that it has taken approximately 108 seconds to build this model. 
```{r}
...
This random forest model has the high accuracy(0.9947).
```{r}
